...
namespace Ruzil3D.Geometry
{
	...
	public struct Plane
	{
		...
		//Допуск, с которым сравниваются плоскости: разность единичных нормалей и относительная разность расстояний
		//до начала координат. Он покрывает погрешность приведения к нормальному виду (несколько ulp), но не больше:
		//с допуском 1e-10 равными оказывались и плоскости на расстоянии 1e12 от начала координат, отстоящие друг от друга на 70.
		...
		/// чтобы погрешность приведения к нормальному виду не делала различными уравнения одной и той же плоскости.
		...
		/// <exception cref="ArgumentException">Если прямая и плоскость параллельны (скалярное произведение нормали и направляющей равно нулю).</exception>
		/// <remarks>Как и прежде, параллельность проверяется точным сравнением с нулём: прямая под очень малым углом к плоскости
		/// (например, при скользящем луче) пересекает её в далёкой точке, и эта точка возвращается.</remarks>
		...
		/// либо плоскости параллельны (векторное произведение нормалей равно нулю), в том числе совпадают.
		/// </exception>
		public static Line3D operator *(Plane plane1, Plane plane2)
		{
			...
			//плоскостями. Как и прежде, параллельность проверяется точным сравнением с нулем: плоскости под очень малым
			//углом пересекаются по далекой прямой, и она возвращается.
		...
		*/
		...
		{
			return
				Math.Abs(normal1.X - normal2.X) <= EqualityTolerance &&
				Math.Abs(normal1.Y - normal2.Y) <= EqualityTolerance &&
				Math.Abs(normal1.Z - normal2.Z) <= EqualityTolerance &&
				Math.Abs(d1 - d2) <= EqualityTolerance*scale;
		}
