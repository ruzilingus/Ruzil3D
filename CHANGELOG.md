...
- `Fraction.Parse` и преобразование из `double`: период принимается, только если он надёжно подтверждён; иначе число округляется, а не усекается (`Parse(2.5, 1)` = 3, раньше 2). Результаты для обычных чисел (0.5, 0.1, 1/3, 2/7, 0.1f) прежние.
