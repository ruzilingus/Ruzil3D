...
namespace Ruzil3D.Curves
{
    ...
    public class EllipticArcCurve : PlanarCurve
	{
		...
		/// <exception cref="InvalidOperationException">Развёртка дуги бесконечна или так велика, что число участков не помещается в <see cref="int"/>.</exception>
		public BezierCurve[] RecastToBezierCurves()
		{
			...
			if (quarters >= int.MaxValue)
			{
				//Без проверки число участков переполняло int, и создание массива выбрасывало OverflowException.
				throw new InvalidOperationException("Развёртка дуги слишком велика для приведения к кривым Безье.");
			}
