...
namespace Ruzil3D.Algebra
{
	...
	public struct Matrix : ICloneable
	{
		...
		/// Используется метод Гаусса с выбором главного элемента по столбцу. Вырожденность определяется так же, как прежде: исключение
		/// выбрасывается, если при исключении без перестановок в столбце не нашлось ненулевого ведущего элемента. Для почти вырожденной
		/// матрицы возвращается результат, точность которого определяется числом обусловленности.
		/// </remarks>
		/// <exception cref="DivideByZeroException">Матрица вырожденная.</exception>
		public Matrix GetInverse()
		{
			var size = Length;

			//Прежде ведущим брался первый ненулевой элемент столбца без выбора главного, поэтому [[1e-20, 1], [1, 1]]
			//обращалась неверно, а каждое действие над строками создавало новые векторы (обращение матрицы 200×200
			//выделяло 155 МБ). Теперь система A·X = E решается на рабочих массивах.
			...
			var source = this;
			Action<double[][], double[][]> fill = (matrix, identity) =>
			{
				for (var i = 0; i < size; i++)
				{
					var line = source[i];
					var row = matrix[i];
					var identityRow = identity[i];
			...
			if (!LinearSystem.Solve(a, inverse, fill))
			{
		...
		/// </returns>
		/// <remarks>Хэш-код, как и прежде, постоянный: элементы матрицы изменяемы (в том числе через копию, делящую те же строки), и хэш-код,
		/// зависящий от значений, делал бы недоступным ключ, изменённый после добавления в <see cref="HashSet{T}"/> или словарь.</remarks>
		public override int GetHashCode()
		{
			//Промежуточная версия вычисляла хэш-код по элементам, и HashSet переставал находить матрицу, изменённую после добавления.
			return 0;
		}
